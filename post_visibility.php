...
add_filter('wp_nav_menu_args', 'seu_edit_nav_menu', 10000);

//add_filter('getarchives_where',	array('social_access_control','filter_getarchives_where'), 10000);

//--------------------------------------------
//next two add filter to wp_nav_menu -> new function for custom nav menus
function seu_edit_nav_menu($args = '') {
	$args['walker'] = new SEU_Walker_Nav_Menu;
	return $args;	
}

class SEU_Walker_Nav_Menu extends Walker {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		//added by RSH
		if (post_should_be_hidden($item->object_id)) return false;
